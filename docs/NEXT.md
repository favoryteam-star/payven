...
## ▶ 다음 세션 시작점 (2026-06-22 마감 기준)
**현재 상태:** 큰 기능 사실상 완료. **라이브 = `cfa6454`**. ⚠️ **`64a5028`(PWA 아이콘 연결 + ADR-033 cleanup 보류)는 로컬 커밋만 됨·미푸시** → **다음 세션 첫 할 일 = `git push origin main`(단독)으로 배포**(iOS 홈 아이콘 반영). 2026-06-22 라이브 배치: 구글 로그인(ADR-029)·이름변경+보관(ADR-030)·뒤로가기 컨텍스트(ADR-031)·내역 진행도(ADR-032). 구글 OAuth는 외부 설정(Google Cloud+Supabase)까지 완료·폰 스모크 통과(M4 인증 종료).

**즉시 할 일 순서:**
1. **`64a5028` 푸시·배포** (위 ⚠️).
2. **폰 스모크(누적, 너 혼자 가능):** 오늘분 — 내역 ⋯메뉴 이름변경/보관(배지) · 뒤로가기(내역→정산→뒤로가 내역 복귀) · 내역 진행도 색(보냈어요 눌러 0/N→✓) · (구글/카카오 로그인 겸사) + 옛 누적분(마이 계좌 CRUD·만들기 자동채움·웨이브2 신원/보냈어요·단위 반올림·수정/삭제).
3. **출시 결정 시 = M6 보안 체크리스트(응급 아님, 공개 직전 한 세션):** 레이트리밋(Upstash env→fail-fast 가드) + 키/토큰 롤(공개 유출 아님 확인됨, 출시 직전 1회) + **구글 프로덕션 게시**(현재 테스트 모드라 소유자/테스트 계정만). cleanup은 제외(ADR-033).
4. **앱 스토어 출시(사용자 결정: M6 후):** 라이브 사이트를 셸로 — 안드로이드 **TWA**(구글 OAuth 그대로 OK)부터 → iOS **Capacitor**(웹뷰서 구글 OAuth 막혀 시스템브라우저 처리 + 애플 4.2 심사 주의). (선택) 커스텀 도메인.
5. (틈날 때) 문서 V2 전체 갱신(CLAUDE/PLAN/ARCHITECTURE가 V0 무로그인 기준 — 무거운 작업).
