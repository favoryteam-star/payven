...
## ▶ 다음 세션 시작점 (2026-06-22 갱신 — 폰 스모크 통과 + M6 레이트리밋 활성)
**현재 상태:** 큰 기능 + 폰 스모크 통과 + **M6 레이트리밋 활성 → 출시 거의 준비됨**. **라이브 = `95dc403`**(= 레이트리밋 프로덕션 fail-fast 가드 + Upstash 연결). 2026-06-22 배치: 구글 로그인(ADR-029)·이름변경+보관(ADR-030)·뒤로가기 컨텍스트(ADR-031)·내역 진행도(ADR-032)·iOS 입력 줌 수정(`2a535c9`, 폼 입력칸 16px·핀치줌은 WCAG 1.4.4 위해 안 잠금)·**레이트리밋 활성(`95dc403`)**·**커스텀 도메인 `payven.kr` 연결(2026-06-22)**. **남은 건 키/토큰 롤(출시 직전) + 앱스토어 패키징(payven.kr 기준)뿐.**
...
6. ▶ **앱 스토어(다음 단계):** **PWA = TWA-ready 확인**(manifest standalone·192/512/512-maskable·theme #0FA177/bg·HTTPS·SW). 안드로이드 **TWA** 먼저 — 권장 **PWABuilder.com**에 **`https://payven.kr`** 넣어 AAB 패키징(서명·assetlinks.json 발급). 필요: **Google Play 개발자 계정($25 1회, 인증 1~2일)**. 받은 SHA-256 지문으로 **`payven.kr/.well-known/assetlinks.json`** 호스팅(라우트 추가는 내가). 스토어 등록정보에 **개인정보처리방침 URL 필수** → **초안 작성됨**: `src/app/privacy/page.tsx`(미배포·미커밋, `[문의 이메일]` 자리표시자만 채우면 배포). 그다음 iOS **Capacitor**(웹뷰 구글OAuth→시스템브라우저 + 애플 4.2 심사 주의).
7. (틈날 때) 문서 V2 전체 갱신(CLAUDE/PLAN/ARCHITECTURE가 V0 무로그인 기준 — 무거운 작업).
