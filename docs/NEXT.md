...
## ▶ 다음 세션 시작점 (2026-06-23 갱신 — 게임 B '쏘기' 완료·확정(모드 칩) → 키롤 → 앱스토어)

**▶▶ 다음 세션 할 일(우선순위 — 이것부터):**
1. **✅ 게임 B '한 명이 다 쏘기' — 완료·확정·라이브.** 사용자 결정=**정산으로 기록**(진 사람→낸 사람 전액) + 진입=**모드 칩 `🎲 쏘기`**(1/N 안 토글과 둘 다 만들어 폰 비교 후 모드 칩 채택, 토글 분기 삭제). [[DECISIONS#ADR-034]], 아래 §"게임 B". **잔여=로그인 생성→정산결과 금액 폰 스모크**(OAuth라 자동검증 불가, 도메인 합/net은 테스트+프리뷰 검증됨). 다음 세션은 ↓2번부터.
