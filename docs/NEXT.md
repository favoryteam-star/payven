...
**▶▶ 다음 세션 할 일(이것부터 — 둘 다 본질적으로 '출시 작업'):**
1. ✅ ~~게임 B '한 명이 다 쏘기'~~ — 완료·확정·라이브([[DECISIONS#ADR-034]], 아래 §"게임 B"). 폰 확인 끝. + 하단 정산하기 버튼 폴리시·항목별 입력 순서(멤버 먼저)도 라이브·폰 확인 끝.
2. **키/토큰 롤** (출시 직전 한 방 — 아래 '키롤 체크리스트' 참조). 대부분 사용자가 대시보드에서, 재배포 후 라이브 write 1회 확인은 내가 거듦.
3. **앱 스토어** (Play 개발자 계정 $25[사용자·인증 1~2일] → PWABuilder에 `payven.kr` → AAB·서명·SHA-256 → `payven.kr/.well-known/assetlinks.json` 라우트[내가] → iOS Capacitor[웹뷰 구글OAuth→시스템브라우저, 애플 4.2 심사]).
