...
## ▶ 다음 세션 시작점 (2026-06-23 갱신 — 출시 준비 완료, 다음=게임 B확장→키롤→앱스토어)

**▶▶ 다음 세션 할 일(우선순위 — 이것부터):**
1. **게임 B 확장 — "누가 다 쏠지 / 한 명이 다 쏘기":** A(잔돈 흡수자 게임, `AbsorberGame`)는 완료. B는 게임으로 **한 명이 전액 부담**(쏘기)을 정하는 것. ⚠️ **A와 분할 의미가 다름** — 한 명이 전액이면 친구 간 정산이 거의 없어짐(loser가 이미 다 냄). → **새 모드인지 / 기존 1·N에 토글인지 / 무엇을 기록(정산결과)하는지를 사용자와 먼저 정한 뒤 구현**(ADR-023 교훈: 모호한 UX는 크게 만들기 전 확인). `AbsorberGame`의 돌림판/사다리 컴포넌트 재사용 가능(결과=한 명 인덱스).
