...
## ✅ 게임 B — "한 명이 다 쏘기"(진 사람→낸 사람 전액) (완료·검증 2026-06-23, `7989ba1`)
...
- **진입 두 방식 = 빌드 상수 `SHOOT_PLACEMENT`('mode'|'toggle', `SettleForm.tsx`):** `'mode'`=별도 모드 칩(1/N·항목별·🎲 쏘기) / `'toggle'`=1/N 안 "똑같이 나누기 ↔ 🎲 한 명이 다 쏘기" 토글. 같은 코어(`isShoot` 파생 + 쏘기 picker)를 공유. **현재 라이브='mode'.** 결정 후 진 분기+상수 삭제.
- **변경:** ModeChips(`'shoot'`+`modes` prop) · AbsorberGame(범용 prompt) · validation(`winnerIndex` 범위 refine) · queries(`quickSharesArray` create/update 공용 + `getEditableGroup` 단일승자 감지 복원) · edit route(winnerIndex 전달) · SettleForm(쏘기 picker·결과 미리보기·낸사람 "누가 먼저 냈어요?"·draft에 winnerIndex/shootOn 보존) · settle 도메인/페이지 **변경 0**(net이 단일 송금으로 자연 수렴).
...
- **검증:** `tsc`·test **69**(쏘기 net/전액/자기쏘기 불변식 +3)·lint·build + **프리뷰 e2e 두 진입**(모드 칩→picker→게임→결과 "철수님이 30,000원 다 쏴요 → 나님에게" / 자기쏘기 "본인이 냈으니 정산할 게 없어요" / 단위·흡수자 섹션 숨김 / 토글 동일). **8에이전트 적대리뷰(money/하드룰/엣지 3렌즈→검증): 확정 실결함 0**(2건은 위 무해 휴리스틱, 13건 정확 확인). 생성/수정 RPC 0-share 허용 확인.
- **잔여:** ①사용자 폰에서 두 진입 비교→하나 선택 ②선택 후 진 분기+상수 삭제+ADR ③로그인 생성→정산결과 금액 폰 스모크(OAuth라 자동검증 불가).

## ▶ 다음 세션 시작점 (2026-06-23 갱신 — 게임 B '쏘기' 라이브, 진입 방식 폰 비교 대기 → 키롤 → 앱스토어)

**▶▶ 다음 세션 할 일(우선순위 — 이것부터):**
1. **게임 B '한 명이 다 쏘기' — 진입 방식 확정(폰 비교 후 한쪽 삭제):** ✅ 코드/검증/라이브 완료(`7989ba1`, 아래 §"게임 B" 참조). 사용자 결정=**정산으로 기록**(진 사람→낸 사람 전액, 기존 보드 재사용). 진입 두 방식을 빌드 상수 **`SHOOT_PLACEMENT`**(`src/components/SettleForm.tsx`)로 만들어 **둘 다 폰에서 보고 별로인 걸 뺄** 예정 — **현재 라이브='mode'(모드 칩 🎲 쏘기)**. 토글(B) 보려면 `SHOOT_PLACEMENT='toggle'`로 바꿔 푸시(1줄). **사용자가 하나 고르면 → 진 분기+상수 삭제 + ADR(쏘기) 한 줄 + 필요시 정산결과 페이지에 '🎯 게임으로 정했어요' 맥락(선택).** ⚠️ **로그인 게이트라 생성→정산결과는 폰 스모크 잔여**(도메인 합/net은 테스트+프리뷰 검증됨).
2. **키/토큰 롤** (출시 직전 한 방 — 아래 '키롤 체크리스트' 참조).
3. **앱 스토어** (Play 개발자 계정 $25 → PWABuilder에 `payven.kr` → AAB → `assetlinks.json` 호스팅[내가] → iOS Capacitor).
