...
**▶▶▶▶▶▶▶▶ 2026-06-28 마감 (이게 최신 — clear 후 여기부터): 제품 마감 5건 라이브 → 마케팅 = 인스타 광고 스모크테스트 착수**

> 이 세션: 제품 자잘한 수정 5건 라이브(전부 `d9b7952`까지) → 전략 대화("어떻게 빠르게 반응 받나") → **결론: payven 고통(복잡정산)은 *잠재*(검색 안 됨)라 검색광고(파워링크) X. 토스 $7 fake-door 선례처럼 인스타 관심사 광고 스모크테스트로 수요부터 검증.** @payven.kr 인스타 계정 셋업 완료. 다음 세션 = 광고 소재 작성 + 실행.

**✅ 라이브 코드 5건 (전부 `SettleForm` 중심, 마이그/도메인/RPC 0):**
- **항목별-first 메시징 정렬**(`c8e9048`, [[DECISIONS#ADR-039]]): 히어로·공유수신 기본펼침·OG 신호·타이틀/매니페스트 = 항목별 주인공(기본 1/N 유지). 6렌즈 감사로 "런타임 표면이 전부 N빵-first"였던 걸 정렬.
- **차수별 '이어서 작성' + 재방문 자동복원**(`5778fe2`+`00166d1`, [[DECISIONS#ADR-040]]): 금액/메뉴 입력 시 `localStorage['payven:draft:wip']` 자동저장 → 다시 열면 폼이 **그대로 채워짐**('이어서 작성 중' 표시 + 새 정산 시작). (배너로 묻기→자동복원으로 이터레이션. 빈 폼 삭제 레이스 수정.)
- **갤럭시 영수증 '촬영' 무반응 수정**(`eea26cc`): 안드 드롭다운 닫기(setTimeout)가 촬영 중 `<input>` 언마운트→onChange 미발화. 옵션 클릭 닫기 제거(handleReceipt가 닫음). **주석에 'setTimeout 닫기 재추가 금지' 가드.** (Vercel 로그상 OCR 요청 0건으로 클라 문제 확정.)
- **첫 멤버='나' 고정**(`d9b7952`, [[DECISIONS#ADR-041]]): 받는 계좌는 항상 멤버0에 붙는데, 첫칸 지우고 자기를 끝에 재추가하면 계좌가 엉뚱한 멤버에 붙어 공유화면에 안 뜸(**실 사용자 제보 + DB 재현**). → 첫 칸 삭제불가·'나' 표시·이름만 변경. `removeMember` i===0 가드.
- **라이브 확정 = `d9b7952`.** (각 건 build·test 90·lint·프리뷰 실측 통과.)

**▶▶ 다음 세션 = 마케팅 스모크테스트 실행 (이번 세션 핵심 결론, [[payven-strategy]] 갱신됨):**
- **결론(왜 검색광고 X):** payven 고통은 *잠재*(사람들이 "메뉴별 정산" 검색 안 함) → **네이버 파워링크 등 검색광고 부적합.** 검색량 있는 "더치페이 계산기"는 1/N 의도=카뱅한테 짐. → **관심사 push 광고가 맞음.** 토스도 2015 전 **$7 페북 fake-door**로 수요부터 쟀음(클릭률 측정). **수요=광고로(그라인드 0), 루프=실제 자리에서. 순서 있음.**
- **채널 결정 = 인스타(메타) 관심사 광고** 스모크테스트. 2순위=네이버 밴드·당근 동네광고. 공짜=긱뉴스·커뮤니티 도움글·릴스(공감 스킷).
- **✅ 셋업 완료:** 구글 `payven.kr@gmail.com` + **인스타 @payven.kr**(프로페셔널/비즈니스, 프사=로고 `public/app-icon-512.png`).
- **▶ 바로 할 일(다음 세션 첫):** ①프로필 바이오+링크(payven.kr) ②**게시물 3개**(훅 A "안 마신 술값, 왜 같이 내?" / B "누가 뭐 먹었는지까지" / C "정산하다 어색해진 적" — 광고는 A 홍보; 이미지=앱 항목별 결과 스크린샷 or 브랜드 카드) ③인스타 광고: 목표=웹사이트방문(payven.kr), 타깃=여행·맛집·모임·캠핑·20–34·한국, 예산 ~₩28k(일7k×4일).
- **측정·GO/KILL(미리 박음):** 🟢 CTR 1.5%+ & 광고기간 정산생성↑ → 루프단계로 / ⚠️ CTR높은데 생성0 → occasion 타이밍 문제(메시지보단 상황) / 🔴 CTR<0.7%&생성0 → 훅·고통 약함, 각도 변경. **정산 생성수 = Supabase 신규그룹으로 추적(클로드가 광고기간 같이 봄).**

**잔여(이전, 유효):** 2026-06-27 루프 측정(실제 정산 1건·viewer→creator)·claim 왕복·OG/촬영 폰 실측. 출시 보류분(기본 모드 항목별 전환·SettleBoard '내가 먹은 항목' 근거·스토어 앱이름/SEO/스크린샷 항목별-first). D-U-N-S 번호 대기 → Play.

상태 = 코드 라이브 `d9b7952` · 마케팅 셋업 완료, 광고 실행 대기.

---

**▶▶▶▶▶▶▶ 2026-06-27: 전략 피벗 + 성장 트랙 A 착수**

> 이 세션: **버그/속도 마무리 → 전략 재정립(거대화 한계 직시) → 콜드 전환 측정 구축 → 키워드 실측(검색=얇음) → 딥리서치(데일리 유틸 후보 전멸) → 결론 = 트랙 A(페이븐을 *잘 퍼지는 좋은 유틸*로) 착수.** 사장님 의도 = "토스처럼 작은 쐐기→확장"인데, **쐐기가 확장되려면 먼저 리텐션/확산이 돼야 함** → 그게 A.
...
> 이번 세션: **영수증 스캔 입력 UX 플랫폼별 처리(특히 갤럭시) + 공유 상세 '흡수자 안내' 신규+버그수정 + 미로그인 OCR 게이트 + 문서 동기화.** 전부 라이브. **코드 최종 `6957263`, 문서 포함 `278c534`.** 폰 확인은 사장님 진행 중.

- **✅ 영수증 스캔 = 플랫폼별 입력**(여러 이터레이션 후 확정): **iOS·데스크톱**=단일 "📷 영수증 스캔" → 누르면 OS 기본 선택 바로 열림(원래 동작). **안드로이드(갤럭시 SM-G991N)**=`accept`만으론 시스템 포토피커(갤러리) 직행해 카메라가 없음 → 누르면 **촬영/앨범 드롭다운**(촬영=`capture=environment` 카메라, 앨범=capture 없음 갤러리), **옵션 클릭 시 즉시 닫힘**(`setTimeout(0)`로 파일창 먼저 연 뒤 닫아 언마운트 레이스 회피). 분기=`lib/ua.isAndroid(ua)`(+test) → `onAndroid` 상태(useEffect, SSR=false 하이드레이션 안전). 위치=메뉴 추가/합치기 **아래**. 커밋 흐름 `3c5474e`(촬영/앨범 2버튼)→`258f6a8`(원래 단일버튼 복귀)→`87e6536`(안드만 드롭다운)→`91d0fec`(옵션클릭 닫힘)→`6957263`(미로그인 게이트). **교훈: iOS는 네이티브 시트가 촬영을 주지만 안드로이드 Chrome은 안 줌 → 플랫폼 분기 필요.**
- **✅ 미로그인 OCR 게이트**(`6957263`): 서버는 이미 Gemini 호출 **전** 로그인 체크라 미로그인=토큰 0(확인). 추가로 **클라에서도** 미로그인이면 스캔 클릭 시 파일 선택 안 열고 로그인 시트(`isLoggedIn` prop 이미 홈 page가 넘김). 사진 업로드 경로 자체 0 → 남용·헛수고 0. **로그인 사유 구분(`loginReason` ref):** 스캔 때문에 로그인하면 입력만 복원·**자동제출 안 함**(정산하기는 기존대로 복원+자동제출). draft에 `reason` 저장, 복원에서 `reason!=='scan'`일 때만 `setAutoSubmit`(옛 draft=무 reason=자동제출, 하위호환).
- **✅ 공유 상세 '흡수자 안내' 신규**(`09f7356`+버그수정 `57a2493`): 항목별 정산을 단위(천원 등)로 맞추면 한 명이 잔돈을 더 냄 — 공유 '상세히 보기'에 **"💡 OOO님이 남은 금액 N원을 더 냈어요"**. **마이그 0015(프로덕션 적용됨): `groups.leftover_amount`(기본 0)+`absorber_index`**(추가 컬럼·기본값이라 기존 영향 0, RPC 안 건드리고 생성/수정 후 `setGroupMeta` best-effort UPDATE). 금액=폼의 '남은 N원'(저장된 leftover_amount, `domain/roundingLeftover` 폼·서버 공용 단일 출처), **사람=분담 역산(`domain/topAbsorber` — 자연 균등분할보다 더 낸 멤버)**. 🐛 **버그:** 처음엔 사람을 `absorber_index`(positional)로 찾았는데 **멤버를 한 트랜잭션에 동시 insert→created_at 동일→`order('created_at')` 불안정** → 엉뚱한 사람(+동명이면 더) 표시("민태욱 흡수인데 나희진으로 떴다"). → 멤버 정렬 의존 제거, 분담(멤버 id)에서 역산. **읽기 경로 수정이라 기존 정산도 링크 새로고침으로 교정.** **한계:** leftover_amount는 **새로 만들거나 '수정' 저장한 정산부터** 채워짐(옛 정산은 안내 없음). SettleDetails 하단에 안내(extra>0일 때만).
- **✅ 문서 동기화**(`278c534`): CLAUDE §스코프 + PAYVEN_PLAN §11에서 **OCR·로그인을 비목표→"원래 비목표였으나 라이브"**로 이동(둘 다 라이브였는데 stale). *잔여: ARCHITECTURE/DECISIONS·CLAUDE 헤더 "무로그인" 프레이밍 등 V0→V2 리프레시는 별도 패스.*
- **domain**: `roundingLeftover(amount,n,unit)`(폼 quickLeftover/itemsLeftover 동일 공식 단일 출처)·`roundingExtras`/`topAbsorber`(분담 역산) 추가. **test 82**(settle+ua). tsc·lint·build green.
- ⚠️ **Vercel 웹훅 누락 1건**: `91d0fec` 직접 push가 배포 안 됨 → **빈 커밋 `0c7603e` 재트리거로 복구**(이 repo 기존 함정, 또 생기면 빈 커밋). 확인은 `list_deployments` `since=`로.
- **▶ 폰 스모크 잔여(사장님):** 갤럭시 촬영=카메라 열림·옵션탭 드롭다운 닫힘 · 미로그인 스캔=로그인 시트 · 흡수자 안내=민태욱(새/수정 정산). ~~"나희진→나희진" 동명 표시 충돌~~ → ✅ **수정·라이브·폰 확인 끝**(아래 2026-06-26 추가).

라이브 = `278c534` (코드 `6957263`).

---

**▶▶▶▶▶▶▶ 2026-06-26 추가 — 동명 표시 충돌 수정 (라이브 `a26a89a`, 폰 확인 이상없음):**

공유 정산은 받는 사람을 **예금주 실명** 우선으로 보여준다(멤버명이 '나'여도 친구가 누군지 알게, `displayName = accountHolder ?? name`, [[DECISIONS#ADR-015]]). 그 예금주명이 **같은 그룹 다른 멤버의 멤버명**과 같아지면(예: 계좌주 예금주 '나희진' + 참여자 '나희진') 송금 행이 `나희진 → 나희진`이 되어 **자기송금처럼** 보였다. `minimizeCashFlow`는 서로 다른 멤버만 짝지으므로 순수 **표시** 충돌.
- **신규 `lib/displayNames.resolveDisplayNames`**(브라우저 안전·순수, +test 8): **동명 충돌일 때만** 구분자를 붙여 유일화 — ①계좌 주인=은행명(`나희진 (국민)`, **방향 무관**이라 예금주가 '낸 사람'으로 나오는 히어로에서도 맞음) ②예금주명이 멤버명을 덮은 경우=원래 멤버명 ③진짜 동명=순번. **충돌 없으면 글자 그대로 동일**(회귀 0).
- `settle/page.tsx`의 `displayName`·메타 `dispName`을 새 해석기로 교체(보드·상세·흡수자·히어로 일관). **스키마·도메인·서버 0**, 읽기 경로 수정이라 **옛 정산도 링크 새로고침으로 교정**(0015 흡수자 안내와 같은 결).
- 검증: **test 90**(82→90)·tsc·lint·build green + 실 프로덕션 그룹 dev 프리뷰(충돌 그룹 `홍길동 → 나희진 (국민)`·신원칩 구분 / 충돌 없는 그룹 불변) + **사장님 폰 확인 "이상없어".** 충돌 그룹 슬러그(참고): `D1Zb4vaPmT3lkXJZurmdi`(한 명이 쏘기)·`OIvtwrrfes9b9sE8-Qp2f`(볼링)·`u4Mp-kZr8pvTtTDKczZux`(항목별 6/25).

라이브 = `a26a89a`.

---

**▶▶▶▶▶ 2026-06-25 (OCR/Gemini 세션) 마감:**

> 이번 세션: **메뉴 카드 UI 개편 + 영수증 OCR 기능(Gemini) 출시 + Gemini 유료(Tier 1) 결제 설정·트러블슈팅.** 전부 라이브. 확정 `98cb7f0`. (운영 함정·세부는 메모리 [[gemini-ocr]]에도 기록.)

- **메뉴 카드 UI**(`de95a28`·`867d7f1`): 항목별 '메뉴별로 나눔'을 **영수증 카드 리스트**로(메뉴마다 둥근 카드·합계 항상표시[0원 흐림]·수량1 중복제거·이름=제목 위계). 이름 칸 **옅은 채움 배경**(수정 가능 affordance). 3방향 디자인 패널→합성.
- **✅ 영수증 OCR 출시**(라이브 `98cb7f0`): 항목별 메뉴 footer **"📷 영수증 스캔"** → 사진(촬영/앨범) → **Gemini 2.5 Flash-Lite**가 `{메뉴명·수량·금액}` 추출 → 메뉴 카드 자동채움. 참여자 배정은 수동(엔진·분담 0변경).
  - `src/server/ocr.ts`(server-only, `GEMINI_API_KEY` 여기서만, 구조화 JSON+정수 KRW `toInt`+503/429 재시도4+Flash 폴백) · `ocrReceiptAction`(로그인 게이트+'ocr' 레이트버킷+zod+`console.error`) · SettleForm(1568px JPEG 축소→action→among=전원, qty>1만 표시, 투명 오버레이 input=iOS 선택메뉴 위치 정상) · privacy에 Google(Gemini) 위탁·미저장 고지 · `maxDuration=60`(홈·edit) · dep `@google/genai`.
  - 커밋: `d8c514b`(기능)·`9d42aff`(수량)·`f4855d5`(촬영+앨범)·`a54bc9c`(iOS 위치)·`98cb7f0`(재시도/폴백/타임아웃/로그).
  - **검증**: 실물 이학갈비 영수증 **100% 정확**(메뉴3·수량·금액·합계 125,000), **장당 ~₩0.13**(입력~350·출력~130토큰), **폰 라이브 확인됨.**
- **✅ Gemini 결제(Tier 1)**: 무료=모델당 **하루 20회(RPD)** 뿐이라 테스트 중 막힘(429). 결제 등록 → Tier 1(**Flash-Lite RPD 무제한·분당 4,000**), **선불 ₩25,000 충전**. 유료=데이터 학습 미사용.
  - **🔑 함정(다음에 또 만남)**: `"prepayment credits are depleted"` = **크레딧 묶인 프로젝트 ≠ 키의 프로젝트.** 해결 = **크레딧 있는 프로젝트('My First Project')에 새 키 → Vercel `GEMINI_API_KEY` 교체 → redeploy.** (가져오기 UI 안 먹힘.)
  - **⚠️ 자동 충전 OFF** — ₩25,000(≈10만 스캔) 소진 시 멈춤. 프로덕션은 **자동충전/후불 켜기 고려.**

**▶ 다음 세션 첫 할 일:**
1. ✅ **문서 동기화 완료(2026-06-26)**: CLAUDE.md §스코프 + PAYVEN_PLAN §11에서 "영수증 OCR=V2 비목표"·"로그인=비목표"(둘 다 라이브)를 제거하고 "원래 비목표였으나 라이브" 블록으로 이동(OCR=Gemini 키 server-only·로그인 게이트·이미지 미저장 명시). *(잔여 V0→V2 문서 리프레시: ARCHITECTURE/DECISIONS·CLAUDE 헤더 "무로그인" 프레이밍 등은 아직 — 별도 패스.)*
2. **(전략) 수요 테스트(C)** — 이전 1순위. OCR("복잡 정산 + 영수증 자동입력")이 차별점 보강 → 이걸로 총무 모집·실사용 검증.
3. (선택) Gemini 자동충전/후불 설정 · 계정삭제 실삭제 폰 스모크(이전 잔여).
...
> 이번 세션: assetlinks 라이브·구글검증 + 계정삭제 경로 + Play 제출 패키지 + **경쟁 재평가(해자 약화)** + **라인 수량 기능 완성**(영속화+공유상세 표시+영수증3행 UI). 라이브 확정 `6afb68a`.

- **✅ assetlinks 라이브**(`618c6b6`) — `public/.well-known/assetlinks.json` 업로드키 지문1, 구글 Digital Asset Links API 통과·에러0. middleware에 `.well-known` 제외. **Play 서명 지문②는 .aab 업로드 후 추가**(아래 출시 단계).
- **✅ 계정·데이터 삭제**(`a6cf75b`) — 마이 탭 "계정 삭제" 2단계(Play 정책). `deleteMyAccountAction`→auth 유저 삭제(저장계좌·내모임 cascade·공유정산 owner null 비식별화). ⏳ 로그인 폰 스모크(실삭제 — 버리는 계정으로).
- **✅ Play 제출 준비** — `docs/play-store-listing.md`(앱이름·설명·데이터보안·콘텐츠등급 복붙 시트) + `store-assets/feature-graphic.html`(열고 'PNG로 저장'→1024×500). 스크린샷은 폰에서 payven.kr 캡처(시트 §6-3).
- **✅ 라인 수량**(마이그 0014 `expenses.quantity`+RPC·`fb04669`/`6afb68a`) — 공유 상세(SettleDetails)에 '단가×수량'(수량>1만), 폼=영수증 3행 '단가 [금액] 수량 [−1+] 합계'. amount=라인총액 불변→분담·test69 0변경. 수정 라운드트립 복원. **OCR+사람별 수량가중=V2 1순위 파킹**(아래 📌).
- **⚠️ 경쟁 재평가(전략 하향·중요)** — 위 §"🧭 전략 결정" 바로 아래 "2026-06-25 경쟁 재평가" 참조. 무로그인·비대칭 빈틈 상당 점유(카뱅 빵나누기·토스·glasswallet[쐐기 카피 그대로]·qwer.dev·suljeongsan·배민 함께주문). 남은 빈틈=깊은차등×무로그인협업보드×토스×저장 한 가닥(medium). **결론: 성장 투자 전 수요검증(C) 먼저.**

**▶ 다음 세션 첫 할 일(우선순위):**
1. **(C) 수요 테스트(1순위)** — 사장님이 **총무 5~10명 모집처**(본인 모임/회사/동아리/지인) 답하면 → 권유 메시지 초안 + 관찰 체크리스트 작성. **C1**=네이버 검색광고 키워드도구로 핵심 키워드 실검색량(사장님, 30분). **C2**=실사용 1건씩(🟢GO=자발적 재사용+viewer→creator 순간 / 🔴KILL="그냥 카뱅 쓰지" 미지근). 결과로 출시·성장 vs 취미출시/피벗 가름. (C3 선택=Vercel Web Analytics, 내가 30분.)
2. **(Android 출시)** — D-U-N-S 번호 메일 오면 → Play Organization 계정($25, makersbridge 3중일치·새 결제프로필) → .aab 업로드 → App integrity의 **Play 앱서명 SHA-256** 복사 → (나) `assetlinks.json` 2번째 지문 추가·재배포 → 테스트기기 주소창 사라짐 확인 → 프로덕션 제출.

⚠️ **dev 함정 재확인**: 프리뷰 스크린샷이 이 앱 SW 때문에 자주 타임아웃 → eval(SW unregister+caches.delete+reload)로 DOM 검증하거나 `mcp__visualize` 렌더로 대체.

---

**▶▶▶ 2026-06-24 마감 현황 — Android 출시 진행 중 (이전 — 위 2026-06-25로 갱신/완료됨, 아래는 배경):**
...
**⚠️ 2026-06-25 경쟁 재평가(딥리서치 104에이전트+정조준 검증 — 위 전략을 하향 수정):** "복잡·비대칭 정산" 빈틈조차 **상당 부분 점유된 레드오션**으로 확인됨. **(1) 무로그인 링크 = 더 이상 해자 아님**: 카카오뱅크 빵나누기(2026-01-20 출시, 앱·계좌 없는 친구에 링크 정산+1·2차 합산+카톡연동)·토스 모임정산(무앱 링크+각자 결제금액 비대칭)이 라이브. **(2) 비대칭·차등 계산도 점유**: [glasswallet(유리지갑)](https://glasswallet.com/calculate/split-bill/)이 **항목별 차등 배정+술 안 마신 사람 제외+늦참 일부+차수**까지 제공하고 **"카카오페이/토스는 단순 N빵, 우리는 항목별 차등"이라는 페이븐이 쓰려던 그 쐐기 카피를 글자 그대로** 사용(단 계산기 팜=SEO 경쟁자). [qwer.dev 술값계산기](https://qwer.dev/playground/drink-calc)=술 비대칭, [suljeongsan](https://suljeongsan.com/)=무로그인+차수+선택참여+늦참비율. **(3) 신규 위협**: **배민 '함께주문' 더치페이**(2026-06-25 ZDNet, 주문~정산 앱내)·앱 다수(A형총무의 더치페이 등). **남은 빈틈=실 한 가닥**: "깊은 차등 계산 × 무로그인 협업 보드(각자 자기 몫 확인) × 토스 송금 × 저장(계좌·모임·내역)"을 *한 몸*에 가진 건 아직 페이븐뿐(빈틈 real, 단 medium·부재증명·각 재료는 개별 상품화됨, 포지셔닝 카피는 glasswallet이 선점). **결론: 차별점이 '기능·포지셔닝 해자'→'제품경험 한 가닥'으로 좁혀짐. 지불 의사(쓸 이유) 미검증 → 성장 투자 전 싼 수요 테스트(C) 먼저.** **딥리서치 미충족**: 키워드 실 검색량(축1)·커뮤니티 채널(축3)은 레이트리밋·검증실패로 미확보 — 네이버 검색광고 키워드도구로 별도 수집 필요.
...
**🧾 영수증 OCR + 수량 가중 = V2 1순위 차별화(2026-06-25 결정, 출시엔 제외):** "복잡 정산 계산기" 포지셔닝의 첫 V2 기능. **데모로 검증 끝** — 실제 한국 영수증(이학갈비, 줄바뀐 단가·구겨짐) Claude 비전이 메뉴 3개+금액 정확 추출, 항목합==총액 자동검증 통과. **비용**=Haiku 비전 ~7원/장·Sonnet ~20원/장(스캔 시에만, 검증 단계 월 1만원 미만). **엔진 기보유**=`splitByWeights`가 가중치 이미 지원(수량 가중=UI 토글만, 도메인 변경 0). **구현 시 필요**=비전 API server 경계+이미지 업로드/검수 UI+남용가드(withRateLimit+로그인+크기캡)+개인정보처리방침·Play 데이터보안 갱신(이미지 외부 전송). **OCR 한계**=메뉴·금액 타이핑만 줄여줌, '누가 뭘 먹었나' 참여자 배정은 여전히 수동(절반의 절약). **착수 신호**=출시 후 실제 사용자가 "영수증으로 넣게 해줘" 요청 뜨면 우선순위 확정(지금은 추정 단계라 제외).
