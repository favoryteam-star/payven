...
- **UI(홈·항목별)**: 단위 칩 [안 함·10원·100원·천원](기본 '안 함'=현행). unit>1+남는금액>0이면 "남은 N원 누가 낼까요?" 멤버 칩(필수, 안 고르면 제출 막힘). 홈=1인당 박스 숨기고 "각자 X·남은 Y", 항목별=인별 합계 tabs 실시간 반영. 로그인 draft에 unit·absorberIndex 보존.
- **검증**: test **58 green**·build·lint. 프리뷰 e2e: 홈 100원→"각자 3,300·남은 100", 천원→"각자 3,000·남은 1,000", 흡수자 미선택 제출→검증 에러, 선택→로그인 게이트 / 항목별 단위 선택 시 tabs 반올림(자동 나 3,400→홍길동 흡수 시 3,400). **생성은 로그인 게이트라 폰 스모크 잔여**(도메인 합==amount라 RPC 경로 불변).
