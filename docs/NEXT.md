...
## ▶ 다음 세션 시작점 (2026-06-22 갱신 — 폰 스모크 통과)
**현재 상태:** 큰 기능 + **폰 스모크(누적) 통과 → 출시 직전 게이트 클리어**. **라이브 = `2a535c9`**(= iOS 입력 포커스 자동확대 수정·폼 입력칸 16px). ✅ `64a5028`(PWA 아이콘)·iOS 줌 수정 모두 푸시·배포 완료(작업 트리 깨끗). 2026-06-22 배치: 구글 로그인(ADR-029)·이름변경+보관(ADR-030)·뒤로가기 컨텍스트(ADR-031)·내역 진행도(ADR-032)·**iOS 입력 줌 수정(폼 입력칸 `text-[15px]`/`text-sm`→`text-[16px]`, 핀치줌은 WCAG 1.4.4 위해 안 잠금)**. **남은 건 운영(M6)+패키징(앱스토어)뿐.**
...
3. ▶ **다음 = 출시 결정 시 M6 보안 체크리스트(응급 아님, 공개 직전 한 세션):** 레이트리밋(Upstash env→fail-fast 가드) + 키/토큰 롤(공개 유출 아님 확인됨, 출시 직전 1회) + **구글 프로덕션 게시**(현재 테스트 모드라 소유자/테스트 계정만). cleanup은 제외(ADR-033).
