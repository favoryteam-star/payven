...
**현재 상태:** 큰 기능 + 폰 스모크 통과 + **M6 레이트리밋 활성 → 출시 거의 준비됨**. **라이브 = `95dc403`**(= 레이트리밋 프로덕션 fail-fast 가드 + Upstash 연결). 2026-06-22 배치: 구글 로그인(ADR-029)·이름변경+보관(ADR-030)·뒤로가기 컨텍스트(ADR-031)·내역 진행도(ADR-032)·iOS 입력 줌 수정(`2a535c9`, 폼 입력칸 16px·핀치줌은 WCAG 1.4.4 위해 안 잠금)·**레이트리밋 활성(`95dc403`)**. **남은 건 M6 잔여 2개(키롤·구글 게시, 둘 다 완전공개 직전 콘솔 작업) + 앱스토어 패키징뿐.**
