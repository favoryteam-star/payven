...
## ✅ 공유 보드 단순화 + 1인당 제거 (완료·검증 2026-06-21)
사용자 피드백 2건. 상세 [[DECISIONS#ADR-023]](ADR-015 일부 대체).
- **① 1인당 제거:** 정산결과 히어로의 "1인당 {금액}" 삭제 — 반올림·흡수자로 6,668/6,666처럼 달라 오해. 커스텀 제목=요약 "총 X · N명", 기본 제목="총 X"가 히어로. `perPerson` 삭제. (만들기 폼 1인당 미리보기는 `leftover===0`만 떠 정확 → 유지.)
- **② 공유 보드 통일:** "링크 받으면 사진처럼 바로 버튼" → `SettleBoard`를 **단일 관리 보드**로(누구나 전 행 토스 송금/보냈어요/취소). **ADR-015 친구 개인화('내가 콕' 신원·"내 차례" 히어로·전체 보기)+`canManageAll` 분기 폐기**, page의 `getAuthUser`도 제거. **안전: '친구는 자기 것만'은 UI 가드였을 뿐, 서버 net 가드가 과다기록 차단(그대로)** → 누가 눌러도 계산 안전. 스키마·도메인·액션 0.
- **검증:** build(settle 4.95→3.79 kB)·lint·test 58. 프리뷰(무로그인): 히어로 1인당 없음·신원 선택 사라짐·홍길동/김철수 행에 [토스 송금][보냈어요] 바로·받는 계좌+복사. **dev SW(`payven-shell-v4`) 옛 청크 → SW 해제+캐시 삭제로 해결.** 잔여: 보냈어요/취소 폰 스모크.
