...
**▶▶▶▶▶▶▶▶ 2026-06-28 (이게 최신 — clear 후 여기부터): 제품 표면 '항목별-first' 메시징 정렬**

> 이 세션: 친구 실사용 피드백("항목별 누가 먹고 안 먹고는 쓸만, 단순 N빵은 토스가 편함")이 2026-06-24 경쟁분석 + 트랙 A를 **현장 검증** → 6렌즈 다중에이전트 감사로 "제품이 실제로 항목별을 주인공으로 내세우나?" 점검 → **판정: 런타임 표면(첫화면·공유수신·OG·타이틀)은 전부 N빵-first, 항목별-first는 미노출 스토어 카피에만 = 메시지 불일치** → 메시징·공유 표면만 항목별-first로 정렬(**기본 모드 1/N 유지**, 사용자 선택).

- **✅ 항목별-first 메시징 정렬**([[DECISIONS#ADR-039]], 코드 6곳·마이그/도메인/RPC 0): ①홈 히어로 "안 마신 술값 빼고, 누가 뭐 먹었는지까지 딱 나눠요" ②공유 수신 항목별 내역 **기본 펼침**+"🧾 누가 뭘 먹었는지 보기"(isItemized만 렌더→1/N 무영향) ③카톡 미리보기 항목별 분기 ④OG 카드 "메뉴 N개·사람마다 다르게"(snap.rounds 재사용·추가쿼리0) ⑤⑥타이틀/매니페스트 "복잡한 정산도 링크 하나로". test 90·lint·build green, 프리뷰 실측(히어로·문서타이틀·매니페스트, 콘솔0).
- **▶ 다음:** 실 항목별 링크 폰 실측(②③④ — 항목별 정산 만들어 카톡 카드+수신 펼침 확인). **보류분(출시 직전):** 기본 모드 항목별 전환 · 송금카드 '내가 먹은 항목' 근거(SettleBoard) · 스토어 앱이름 1순위·SEO 롱테일 태그·스크린샷 항목별-first 재배열(play-store-listing).
- **잔여(이전, 그대로 유효):** 2026-06-27 루프 측정(실제 정산 1건·viewer→creator)·무로그인 생성→claim 왕복·OG 카톡 실측·OCR 속도. D-U-N-S 번호 대기 → Play.

- **✅ 차수별 '이어서 작성'**([[DECISIONS#ADR-040]], `SettleForm`만): 1차→2차 시간차 입력을 **로컬 초안 자동저장 + 재방문 '이어서 작성' 배너**로(공유 전 로컬이라 DB·익명수정 무충돌). 매번 저장 버튼 없음. 마운트 로그인draft 우선→WIP, 생성성공·새로시작·7일만료 시 비움. 프리뷰 5경로 실측·레이스 1건 수정(빈 폼 자동저장이 방금 감지한 초안 삭제→빈 폼 삭제 안 함).

상태 = 확정·검증·라이브(항목별-first 메시징 `c8e9048` + 차수별 이어서 작성 = 이 배치).
