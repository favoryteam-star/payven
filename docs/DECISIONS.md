...
### ADR-039 — 제품 표면을 '항목별-first' 메시징으로 정렬 (기본 모드는 1/N 유지)
- **맥락(2026-06-28, 전략 피벗 후속):** [[ADR-038]] 루프 강화 + 2026-06-24 경쟁분석 결론 = 차별 쐐기는 **복잡·비대칭 정산(항목별)**, 단순 1/N은 토스/카카오페이(결제 레일+송금 이력) 못 이김. 친구 실사용 피드백도 독립적으로 동일("항목별은 쓸만, 단순 N빵은 토스가 편함"). 그런데 6렌즈 다중에이전트 감사 결과 **제품 런타임 표면(첫 화면·공유 수신·OG 카드·앱 타이틀)은 전부 N빵-first**, 항목별-first는 아직 미노출 스토어 카피(`play-store-listing`·`feature-graphic`)에만 → 광고는 항목별을 파는데 앱은 1/N을 보여주는 **메시지 불일치**.
- **결정:** 메시징·공유 표면을 항목별-first로 정렬하되 **기본 모드는 1/N 유지**(사용자 선택). 근거 = "1/N=진입 미끼(현관문 마찰 0), 항목별=쐐기" → 현관문은 그대로, **광고판(카피·공유·수신)만 전부 항목별 주인공으로**. 기본 모드를 항목별로 여는 건 흔한 단순 N빵에 마찰(차수·메뉴 세팅)을 늘려 보류.
- **변경(코드 6곳, 마이그·도메인·RPC 0):** ①홈 히어로 `1초 정산`→"안 마신 술값 빼고, 누가 뭐 먹었는지까지 딱 나눠요"(`SettleForm`). ②공유 수신 '항목별 내역' **기본 펼침**+라벨 "🧾 누가 뭘 먹었는지 보기"(`SettleDetails` — `page` `showDetails=isItemized`라 항목별일 때만 렌더 → 1/N 무영향). ③카톡 미리보기 `description` 항목별 분기("메뉴별로 각자 먹은 만큼…", `settle/page`). ④동적 OG 카드 항목별 신호("메뉴 N개·사람마다 다르게"+푸터 "내 메뉴만큼만…", `opengraph-image` — `snap.rounds` 재사용, 추가 쿼리 0). ⑤앱 타이틀/설명 "페이븐 — 복잡한 정산도 링크 하나로"(`layout`). ⑥매니페스트 동일 카피(`manifest`).
- **검증:** test **90**·lint·build(server-only 누수0) + 프리뷰 실측(홈 히어로·문서 타이틀·매니페스트 fetch, 콘솔0). ②③④는 항목별일 때만 작동(빌드 타입체크 보증) — 실 항목별 링크 폰 실측 잔여.
- **보류·후속:** 기본 모드 항목별 전환 / 송금 금액 카드에 '내가 먹은 항목' 근거(`SettleBoard`) / 스토어 앱이름 1순위·SEO 롱테일 태그·스크린샷 항목별-first 재배열(`play-store-listing`) — 전부 미적용, 출시 직전 검토.
- **상태:** 확정·검증·라이브(`c8e9048`). PAYVEN_PLAN §1 동기화 완료.

### ADR-040 — 차수별 '이어서 작성'(작성 중 로컬 초안 + 재방문 자동 복원)
- **맥락(2026-06-28):** 실제 회식은 1차→2차→3차가 시간차로 벌어지는데 폼은 한 번에 다 입력해야 해 새벽에 몰아 재구성(기억 소실)해야 했다 — 항목별(쐐기)의 가장 큰 입력 마찰. 사용자 요청 = "매번 저장보다 나은 방법".
- **결정:** 공유(생성) **전** 로컬 초안 자동저장 + **재방문 시 자동 복원**(폼이 그대로 채워져 있고, 위에 '이어서 작성 중' 표시 + '새 정산 시작' 탈출구). *공유 후* 차수 추가(라이브 정산 편집)는 익명 수정 불가([[ADR-038]])·교체형 수정이라 아키텍처와 충돌 → V1 제외. 실무상 정산은 끝나고 공유하므로 "초안으로 모아 마지막에 공유"로 충분.
- **변경(`SettleForm`만, 마이그·도메인·RPC 0):** 로그인 왕복 draft([[ADR-017]]) 직렬화 모양 재사용. 금액/메뉴가 들어가면 `localStorage['payven:draft:wip']`에 자동저장(수동 버튼 없음). 마운트 시 ①로그인 draft(sessionStorage·우선) ②WIP 초안(있으면 `applyDraft`로 자동 복원 + '이어서 작성 중' 표시) 순. 생성 성공·'새 정산 시작'(removeItem+리로드)·7일 만료 시 비움. `applyDraft` 추출로 로그인 복원·자동 복원 공용.
- **함정·수정:** 자동저장이 리로드 직후 빈 폼에서 '방금 감지한 초안'을 `removeItem`하는 레이스(첫 커밋 state null + StrictMode 이중호출). → **빈 폼일 땐 삭제 안 함**으로 수정(비우기는 생성성공·새 정산 시작·만료가 담당). 프리뷰로 발견.
- **이터레이션:** 처음엔 '빈 폼 + [이어서 작성] 배너'(복원 전 묻기)로 출시(`5778fe2`)했으나, 사용자 피드백 = "다시 열면 빈 폼+이어하기라 *초기화된 것처럼* 보인다" → **자동 복원**으로 변경(구글 독스식, 돌아오면 그냥 거기 있음). 묻지 않고 채워두고 새로 만들 때만 '새 정산 시작'.
- **검증:** build·test **90**·lint green + 프리뷰 실측(자동저장·재방문 자동복원[폼 채워짐]·새 정산 시작 초기화)·콘솔0.
- **상태:** 확정·검증·라이브.

### ADR-041 — 멤버 0 = '나' 고정(삭제 불가·이름만 변경) = 받는 계좌 앵커
- **맥락(2026-06-28):** 받는 계좌는 RPC가 항상 멤버 0(member_names[0])에 붙인다('내 계좌만' 모델, [[ADR-013]]). 폼이 첫 칸을 '나'로 깔아 그 가정을 만족시키지만, 사용자가 **첫 칸을 ✕로 지우고 자신을 새 칸으로 끝에 다시 추가**하면(이름을 그룹 별명으로 바꾸려고) 멤버 0이 친구로 바뀜 → 계좌가 엉뚱한 멤버에 붙고, 그 멤버가 '받는 사람'이 아니라 공유 화면에 계좌가 아예 안 뜸. **실 사용자 제보 + DB로 재현 확인**(계좌가 의도한 본인 아닌 다른 멤버에 부착).
- **결정(사용자 선택 A):** 첫 칸을 '나'로 **고정** — ✕(삭제) 제거 + 우측 '나' 표시, 이름은 그대로 변경 가능. 별명으로 바꿔도(글자만 수정) 계좌가 항상 본인(멤버 0)에 남는다. 대안 B(계좌 '받는 사람' 멤버 선택 UI)는 보류(더 큼, RPC/마이그 필요).
- **변경(`SettleForm`만, 마이그·도메인·RPC 0):** 멤버 행 트레일링 = i===0이면 '나' 라벨(✕ 없음), 친구면 ✕(멤버>2일 때), 그 외 정렬용 빈칸. `removeMember`에 `i===0` 가드. 이름 in-place 변경은 기존대로. **future: ✕를 멤버 0에 다시 넣지 말 것.**
- **한계:** 계좌는 여전히 '받는 사람으로 등장할 때만' 표시(자기 계좌 자기한텐 안 뜸, SettleBoard `accountIsReceiver`) — 본인이 돈 받는 쪽일 때 친구에게 보인다. 멤버 목록에서 본인 '위치'는 못 옮김(첫 칸 고정).
- **검증:** lint·build·test **90** green + 프리뷰 실측(첫 칸='나'·✕없음·삭제불가 / 친구만 ✕ / 콘솔0). 실기기 재확인 = 사장님.
- **상태:** 확정·검증·라이브.
