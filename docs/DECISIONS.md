...
### ADR-023 — 공유 정산 보드 단순화: 누구나 관리 보드 + 1인당 표시 제거 ★ (ADR-015 일부 대체)
- **맥락(사용자 피드백 2026-06-21):** ①정산결과 히어로의 "1인당 {금액}"이 반올림·흡수자([[ADR-016]])로 사람마다 달라 오해 소지(6,668 vs 6,666인데 "1인당 6,666"). ②"링크를 공유 받았을 때 사진처럼 똑같이 버튼 있게" — 친구가 링크 열면 신원 선택 후에야 버튼이 보이던 걸, 바로 주최자와 같은 관리 보드(토스 송금/보냈어요/취소)를 보게.
- **결정 ① 1인당 제거:** 정산결과 히어로에서 1인당 빼고 **총액·인원만**. 커스텀 제목이면 요약 줄 "총 X · N명", 기본 제목이면 "총 X"가 큰 히어로(기존 1인당 자리). `perPerson` 계산 삭제. (만들기 폼의 "1인당" 미리보기는 `leftover===0`일 때만 떠서 항상 정확 → 유지.)
- **결정 ② 공유 보드 통일:** `SettleBoard`를 **단일 모드(전체 관리 보드)**로 — 링크 받은 누구나 전 행에 토스 송금/보냈어요/취소. **[[ADR-015]]의 친구 개인화('내가 콕' 신원 localStorage·"내 차례" 히어로·전체 보기 토글)와 주최자/친구 분기(`canManageAll`)를 폐기.** page에서 `getAuthUser`/`canManageAll`도 제거(보기에 인증 불필요).
- **안전 근거:** [[ADR-015]]의 '친구는 자기 것만'은 **UI 가드일 뿐**이었고(친구 신원=localStorage라 서버는 검증 불가), 과다기록·역방향 차단은 **서버 net 가드**(`recordSettlement`: `fromOwes≥amount && toOwed≥amount`)가 담당. 그 가드는 그대로라 **누가 어느 행을 눌러도 계산은 안전**. 공유 정산은 친구끼리라 협업 편집이 자연스럽다(사용자 판단). 스키마·도메인·액션 변경 0(컴포넌트·page 렌더만).
- **검증:** build(`/g/[slug]/settle` 4.95→3.79 kB)·lint·test 58. 프리뷰(무로그인=친구): 히어로 "사우동 1차 · 총 20,000원 · 3명"(1인당 없음)·신원 선택 사라짐·홍길동/김철수 행에 [토스 송금][보냈어요] 바로 노출·받는 계좌+계좌 복사. **주의: dev에서 PWA SW(`payven-shell-v4`)가 옛 청크 서빙→변경 안 보임 → SW 해제+캐시 삭제로 해결(프로덕션은 콘텐츠 해시라 안전).** 잔여: 보냈어요/취소 왕복 폰 스모크(액션 자체는 웨이브2에서 검증됨).
- **상태:** 확정·검증(라이브 배포 대기). 마이그레이션 없음.
