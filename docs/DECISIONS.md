...
### ADR-040 — 차수별 '이어서 작성'(작성 중 로컬 초안 + 이어쓰기 배너)
...
- **결정:** 공유(생성) **전** 로컬 초안 자동저장 + 재방문 시 '이어서 작성' 배너. *공유 후* 차수 추가(라이브 정산 편집)는 익명 수정 불가([[ADR-038]])·교체형 수정이라 아키텍처와 충돌 → V1 제외. 실무상 정산은 끝나고 공유하므로 "초안으로 모아 마지막에 공유"로 충분.
- **변경(`SettleForm`만, 마이그·도메인·RPC 0):** 로그인 왕복 draft([[ADR-017]]) 직렬화 모양 재사용. 금액/메뉴가 들어가면 `localStorage['payven:draft:wip']`에 자동저장(수동 버튼 없음). 마운트 시 ①로그인 draft(sessionStorage·우선) ②WIP 초안(있으면 배너로 '이어서/새로') 순. 생성 성공·'새로 시작'·7일 만료 시 비움. `applyDraft` 추출로 로그인 복원·이어쓰기 공용.
- **함정·수정:** 자동저장이 리로드 직후 빈 폼에서 '방금 감지한 초안'을 `removeItem`하는 레이스(첫 커밋엔 배너 state가 null + StrictMode 이중호출). → **빈 폼일 땐 삭제 안 함**으로 수정(비우기는 생성성공·새로시작·만료가 담당). 프리뷰로 발견·수정.
- **검증:** build·test **90**·lint green + 프리뷰 5경로 실측(빈폼 배너X·입력 자동저장·리로드 배너O[복원 전]·이어서 복원·새로시작 폐기)·콘솔0.
- **상태:** 확정·검증·라이브.
