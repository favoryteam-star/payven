...
## 1. 포지셔닝 (2026-06-28 갱신 — 피벗 반영, [[DECISIONS#ADR-039]])

페이븐 = 무로그인 링크로 시작하는 친구·모임 정산 웹앱. 첫 출시작.
**차별 쐐기 = 복잡·비대칭 정산**(항목별: 안 마신 술값 빼기·메뉴별·차수별) — 카톡/토스로는 귀찮은 "누가 뭐 먹었는지"를 링크 하나로. 단순 1/N(빠른정산)은 토스/카카오페이(결제 레일+송금 이력 보유)를 못 이기므로 **차별 축이 아니라 진입 미끼이자 기본 모드**로만 둔다 — 현관문(기본 1/N)은 마찰 0, 광고·공유·수신 카피는 전부 항목별을 주인공으로. 친구 실사용 피드백도 같은 결론("항목별은 쓸만, 단순 N빵은 토스가 편함").
**고빈도 = 리텐션**은 여전히 유효 — 가벼운 한 끼(1/N)로 들어와 복잡한 자리에서 항목별로 묶인다.
칼 두 개: **항목별 비대칭 분할(쐐기)** + 무로그인 링크 루프(확산, [[DECISIONS#ADR-038]]).
...
결제·송금 레일 · 입금 자동확인 · 회비/총무(B2B) · 다중통화(V2) · 푸시 알림 · 실시간 동기화(V2).

> **원래 비목표였으나 출시로 당겨온 것(라이브):**
> - **로그인**(M4 — 카카오·구글, *만들기* 게이트. **보기·공유는 여전히 무로그인**).
> - **영수증 OCR**(라이브 `98cb7f0`) = 영수증 사진 → 메뉴·금액·수량 자동입력(Gemini 2.5 Flash-Lite). "복잡 정산 계산기" 차별점으로 당겨옴. **분담 엔진·도메인 변경 0**(인식 결과를 항목별 메뉴에 채울 뿐, 참여자 배정은 수동). server-only `src/server/ocr.ts`에서만 `GEMINI_API_KEY`, **로그인 게이트**(미로그인은 Gemini 호출 전 차단 = 토큰 0) + `withRateLimit('ocr')` + zod, **이미지 미저장**(개인정보처리방침에 Google 위탁·미저장 고지). 플랫폼별 입력: iOS/데스크톱=OS 기본 선택, 안드로이드=촬영/앨범 드롭다운(`accept`만으론 갤러리 직행이라).
